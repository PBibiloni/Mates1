...
df = data.frame(
  ...
  concentracio=c(44, 36, 32.5, 28, 26.3)
)
xtable(df)
ggplot(df, aes(x=temps, y=concentracio)) +
    scale_y_continuous(trans='log10') +
    labs(x='Temps [min]', y='Concentració [mg/100 ml]') +
    geom_point() +
...
model = lm(concentracio~temps, data=df) # Calcular model lineal
abline(model)                           # Dibuixar recta
...
## Aplicacio: dibuixar gràfica lineal amb R
...
# Opció 3
df = data.frame(x=0:10, y=2*4.5^(0:10))
ggplot(df, aes(x, y)) +
    scale_y_continuous(trans='log10') +
    labs(x="Eix x", y="Eix y") +
    geom_line()
